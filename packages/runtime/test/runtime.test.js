...
test("render supports each and if/elif/else blocks", () => {
  const output = render(`
    {#if score > 10}<b>high</b>{:elif score > 0}<b>low</b>{:else}<b>zero</b>{/if}
    <ul>{#each items as item, index}<li>{index}:{item}</li>{/each}</ul>
  `, { score: 2, items: ["a", "b"] });
